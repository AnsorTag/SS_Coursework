...
# Function to denoise an image using the NLMS algorithm
def nlms_denoise(original_image, noisy_image, filter_size=(3, 3), step_size=0.01, num_iterations=10, convergence_threshold=1e-6, regularization_term=1e-6):
    ...
    for iteration in range(num_iterations):
        ...
        # Calculate errors and update filter weights using NLMS rule
        ...
        for i in range(h):
            for j in range(w):
                ...
                input_power = np.sum(patch**2) + regularization_term
                filter_weights += (step_size / input_power) * errors[i, j] * patch
...
def adjust_filter_size(original_image, noisy_image, max_filter_size=(5, 5), step_size=0.01, num_iterations=10):
    ...
    for filter_size_h in range(1, max_filter_size[0] + 1):
        for filter_size_w in range(1, max_filter_size[1] + 1):
            ...
            try:
                denoised_image = nlms_denoise(original_image, noisy_image, filter_size=filter_size, step_size=step_size, num_iterations=num_iterations)
...
denoised_image = nlms_denoise(original_image, noisy_image, filter_size=adapted_filter_size)
